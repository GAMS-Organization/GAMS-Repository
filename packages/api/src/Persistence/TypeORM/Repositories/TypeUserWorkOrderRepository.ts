...
export default class TypeUserWorkOrderRepository extends TypeRepository implements IUserWorkOrderRepository {
  ...
  public async findOneById(id: number): Promise<UserWorkOrder> {
    return await this.repository(UserWorkOrder).findOne({ where: { id: id }, relations: ['user'] });
  }
  ...
  public async countByUserId(userId: number): Promise<number> {
    return await this.repository(UserWorkOrder).count({ where: { user: userId }, relations: ['user'] });
  }
